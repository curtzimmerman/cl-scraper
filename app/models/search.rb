class Search < ActiveRecord::Base
	...
	def refresh
		...
		self.update_hits(self.city, current_hits)		
		...
		search_cities.each do |city|
			self.update_hits(city, current_hits)
		end
	...
	def update_hits(city, current_hits)
		...
		doc.css("div.rightpane div.content p.row").each do |row|
			if current_hits.nil? || !current_hits.include?(row['data-pid'])
				new_hit = self.hits.create(
					...
					checked: true)
				current_hits.push(new_hit.data_pid)
			elsif
